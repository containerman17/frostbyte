...
import {
    MetricResult,
    backfillIncrementalMetric,
    backfillCumulativeMetric,
    getPreviousTimestamp,
    normalizeTimestamp
} from "./utils";

export function handleIncrementalMetricQuery(
    indexingDb: SQLite.Database,
    metricId: number,
    ...
    const results = indexingDb.prepare(query).all(...params) as MetricResult[];
...
export function handleCumulativeMetricQuery(
    ...
    const mostRecentResult = indexingDb.prepare(
